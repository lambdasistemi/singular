...
- [ ] T-atomic-save (IMPLEMENTED, UNEXERCISED): the mirror is written via temp file plus rename under `bracketOnError`. Nothing in the candidate interrupts, crashes or cancels a process during that write, so crash safety is not established — the gate's byte-preservation assertions are all taken across a named refusal, which is a different observable. Implemented and unexercised, not verified. Missing control recorded as FU-107-01.
...
Independent audit of the candidate **and** the changed gates is required
before trunk; the implementer does not accept its own work.

There is no `gate.sh` in this ticket. The gates actually invoked, and
reproducible by an independent party, are:

| Gate | Working directory | Notes |
| --- | --- | --- |
| `nix develop --quiet -c just follower-e2e` | `offchain/` | real-node follower scenario and named-refusal controls |
| `nix develop --quiet -c just e2e` | `offchain/` | full hermetic suite |
| `nix build .#checks.x86_64-linux.cage-tests` | repository root | executes the offchain unit binary |
| `bash lint-changes.sh <BASE>` | `offchain/` | changed-file fourmolu/hlint; BASE is the PR merge base |
| `bash test/lint-selection.sh` | `offchain/` | positive controls for the changed-file selector |
| `bash follower-cli-check.sh` | `offchain/` | executes the published `deployment follow` command and `--rebuild` |
| `nix develop --quiet -c just ci` | repository root | model, simulator, browser, docs, site and presentation |

Historical verification stays bound to the candidate it was run against.
