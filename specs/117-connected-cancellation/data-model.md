...
```mermaid
flowchart LR
  R[Registration receipt] -->|commits output origin and datum| P[Native request]
  R -->|Insert hash authenticates| C[Pending claim]
  R -->|fixes full address| F[Refund destination]
  C -->|same creation transaction| P
```
