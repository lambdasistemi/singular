...
### X3 — the existing journeys follow the contract (amendment of 2026-09-18)

The journeys already on `main` — `li01`, `li-refusals`, `lmlc`, `recovery`,
`retirement`, `retire-verify`, `repair` and their CI jobs — are model-bound
consumers of the cage and naming contracts, like the simulator was for #156.
They are **re-cut in this ticket**, not left to #158: a boot with the four
derived pins; requests carrying an approval and a destination; the record
created by the fold instead of a claim; retirement proved by the recovery key
or the quorum. Rows whose behaviour the design retires — the claim-and-cancel
`LC` rows, the pending-claim `Fold`, retirement by the control key alone —
are retired with a recorded mapping in the journey's README, never silently
dropped and never kept green by a stub. Rows that survive (`LI`, `LM`, `LR`,
`LT` minus the retired ones) replay against the new blueprint with their
attribution checks intact. `main` is never red on a journey job after this
ticket merges. **No new journey is authored here**: `witness-rows` and the
archive are #158's.
