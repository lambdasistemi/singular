...
A registry has an authenticated identity and a map from keys to payload-free `Active` or `Over` values. An absent key has no entry. A representative NFT identifies the active registration of a key within its registry. A request UTxO carries a native request token; Update/Delete requests also carry the existing representative. A certification policy provides accepted application approval for Insert. The application spending script governs legal spends of the application's NFT UTxO.

Registry identity, representative policy, request policy, certification policy and application spending script are distinct semantic roles. Their actual script hashes may coincide if an implementation shares a script; this specification neither requires nor forbids that sharing.
...
A recognized request MUST carry a token minted under the bound Singular request policy. That policy MUST validate native request structure, operation, exact registry/key binding and required custody in the creating transaction. A token issued by an arbitrary substitute policy MUST NOT be accepted as a native request token.

The application constructs the transaction; Singular's policy governs native request minting. Request creation MUST be supportable without observing the mutable registry UTxO. This requirement does not prohibit application inputs, transaction-context checks or authenticated registry configuration.
...
- accepted certification policy identity and evidence;
...
The accepted certifier MUST be authenticated and bound to the registry's rules. A requester MUST NOT gain approval by choosing an arbitrary self-issued policy. The certification mechanism MUST prevent substitution of any bound field after approval.

Application certification and native request minting MAY participate in the same transaction. Approval MUST be established when the native Insert request token is minted; this requirement is not deferred to folding. Certification MUST NOT be treated as a reservation of the key or a guarantee of inclusion/order.
...
Insert requests MUST support withdrawal before successful folding. Withdrawal MUST NOT change registry state or mint a representative. The concrete cancellation authority, conditions, refund handling and token/certificate disposal are unresolved under D3 below; this requirement does not authorize arbitrary third-party cancellation or arbitrary access to deposits.
...
| --- | --- | --- |
| D1 | Registry identity/configuration and policy binding, including hash dependencies | Authenticate accepted certifiers and native policies without assuming a requester-selected issuer is trusted; no global allowlist or specific hash-cycle solution is selected |
| D2 | Logical-to-wire encoding and request/certificate token lifecycle | Bind the whole proposal and required output effects; prevent acceptance after completion; specify certificate reuse/consumption and native request-token disposal |
| D3 | Insert withdrawal authority, cancellation conditions and refund disposition | Keep withdrawal available without permitting asset theft or allowing arbitrary outsiders to repeatedly cancel valid registration attempts; application-authorized cancellation is an option, not an adopted rule |
