...
li01 = do
    ...
    -- from the two partitions' script identities, so the naming
    -- blueprint is as much an input to this row as the registry one.
    ...
    case ( extractCompiledCode "state.state" bp
        ...
        (Just stateBytes, Just requestBytes, Just appBytes, Just witnessBytes) ->
            runLi01 control si stateBytes requestBytes appBytes witnessBytes
        _ ->
            failWith
                "state.state, request.request, application.application \
                \or witness.witness.mint compiled code not found in the \
                \blueprints"
...
runLi01 ::
    ...
    SBS.ShortByteString ->
    SBS.ShortByteString ->
    {- | the naming application validator, whose own hash is the
    application policy pin
    -}
    SBS.ShortByteString ->
    {- | the @witness(kind, registry)@ validator, applied at kinds 0, 1
    and 2 for the three token-policy pins
    -}
    SBS.ShortByteString ->
    IO ()
runLi01 control si stateBytes requestBytes appBytes witnessBytes = do
    withNode $ \sess -> do
            ...
            derivedRequestHex =
                hex . scriptHashBytes . computeScriptHash $
                    applyRequestParams
                        ...
                        requestBytes
        -- #157 D-BOOT: the four pins this registry identity carries,
        -- all derived — NYA applied to the ordinary request-validator
        -- hash, and `witness(kind, registry)` applied at kinds 0, 1
        -- and 2.
        let appApplied =
                appliedApplicationBytes
                    (scriptHashBytes appliedHash)
            ...
            registryId =
                registryAssetId (scriptHashBytes appliedHash) seedName
            witnessPin kind =
                SBS.toShort
                    ( scriptHashBytes
                        ( computeScriptHash
        ...
        emit
            ...
            ( "applicationPolicy 0x"
                <> hex (scriptHashBytes appHash)
                <> " (NYA applied to the ordinary request-validator hash); \
                   \absentPolicy 0x"
                <> hex (SBS.fromShort (witnessPin 0))
                <> "; activePolicy 0x"
                <> hex (SBS.fromShort (witnessPin 1))
                <> "; terminalPolicy 0x"
                <> hex (SBS.fromShort (witnessPin 2))
                <> " — witness(kind, registry) at kinds 0, 1 and 2 for \
                   \registry 0x"
                <> hex registryId
            )
        emit
        ...
        let cfg =
                CageConfig
                    ...
                    , -- #157 D-BOOT: all four derived above for THIS
                      -- registry identity, never typed.
