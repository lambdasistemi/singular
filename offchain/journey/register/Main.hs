...
Ledger realisation (offchain\/naming-correspondence.md, t77 entries):
  ...
  * the representative is minted under
    ...
    policy hash and registry asset id (2 parameters), named @blake2b_256(spelling)@
...
runMode mode namingPath registryPath = do
    ...
    withNode $ \sess -> do
        ...
        appBytes <- if mode == ConnectedCancellation then do
            unless (isNothing mDeployment) $ failWith "connected-cancellation requires isolated devnet"
            let token = TokenId (AssetName (SBS.toShort (deriveAssetName seedRef)))
        ...
        emit
            ...
            ( "naming application 0x"
                ...
                <> " applied 0x"
                <> repAppliedHex
                <> " (parameters: application-policy hash and registry asset id); registry state \
                   \unapplied 0x"
                <> stateUnappliedHex
                <> " applied 0x"
                <> appliedStateHex
        ...
        Just 1 ->
            emit
                ...
                ( "fold "
                    ...
                    <> envAppHex env
                    <> "; registry token 0x"
                    <> hex (case envTok env of TokenId (AssetName n) -> SBS.fromShort n)
                    <> ")"
                )
        other ->
            failWith
                    ...
                    , ConnectedMint
                        ...
                        , cmAssets =
                            Map.singleton
                                (AssetName (SBS.toShort repName))
                                1
                        , cmRedeemer = RawRedeemer mintRedeemer
                        , cmScript = mintScript
                        }
                    ]
                , cfaOutputs = [recordOut]
        ...
        Rejected reason -> do
                ...
                failWith
                    ( rowName
                        <> ": expected-rejection-reason <phase-2 PlutusFailure \
                           \naming the expected validator> but the node \
                           \rejected with <"
                        <> reasonText
            ...
            emit
                ...
                ( rowName
                    ...
                    <> modelReason
                    <> "; phase-2 PlutusFailure naming the expected validator \
                       \0x"
                    <> expectedScript
                    <> "): "
                    <> reasonText
