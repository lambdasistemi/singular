...
Executes the in-scope contract rows @LT02, LT03, LT05, LT06, LT08,
...
Ledger realisation (offchain\/naming-correspondence.md, t66 entries):
  ...
  * authorization is the committed recovery key, revealed and signing,
    or at least `threshold` distinct stored quorum members among the
    required signers (@LT02@ carries no controller signature, @LT03@ is
    one distinct signature short). The control key alone is no route
    since #157 (D-TERMINATE), and the row that stood for it is retired;
    the journey README maps it.
  ...
  * @LT09@ retires a record a real earlier @LT02@ already consumed, so
    the ledger itself refuses the spent output.
...
runMode mode blueprintPath registryPath = do
    ...
    repUnappliedBytes <- case extractCompiledCode "witness.witness" bp of
        Just bytes -> pure bytes
        Nothing ->
            failWith
    ...
    withNodeForPlannedFunding $ \sess -> do
        ...
        let repUnappliedHex =
            ...
            witnessAt kind =
                SBS.toShort
                    ( scriptHashBytes
                        ( computeScriptHash
                            (applyBytesParam registryId (applyIntParam kind repUnappliedBytes))
                        )
                    )
            ...
            repAppliedBytes =
                applyBytesParam registryId (applyIntParam 1 repUnappliedBytes)
        ...
        (cfg, tok) <- case attached of
            Nothing ->
                bootRetirementCage seedRef prov submit tm stateBytes requestBytes (SBS.toShort (scriptHashBytes appHash)) activePolicyPin absentPolicyPin terminalPolicyPin evDir evNext
            Just (path, att) -> do
                emit
                    "attached"
        ...
        if lifecycle
            then do
                ...
                emit "setup" "creating the retirement records (accept2, refusals, duplicates, rr1, rr2, over)"
                ...
                emit
                    ...
                    ( "records live at the application validator 0x"
                        ...
                        <> "; each carries the active witness token of its \
                           \own registry key"
                        <> " under the applied active policy 0x"
                        <> repAppliedHex
                        <> " with control 0x"
                ...
                case mode of
                    MainRun -> runRows env recAccept2 recRefusals recDuplicates recR1 recR2 recOver
                    ControlValid -> runControlValid env recRefusals
                    ControlWrongReason -> runControlWrongReason env recDuplicates recRefusals
...
fundPublicLifecycle env = do
    ...
    let pp = envPp env
        ...
        insertDeposit = Lifecycle.requestDeposit pp (envCfg env) (envTok env) genesisAddr "rt-over" (OpInsert leafActive) now
        fund = Lifecycle.fundedOutput pp refs genesisAddr
        collateral = Lifecycle.collateralOutput pp refs genesisAddr
        retireDeposit = Lifecycle.requestDeposit pp (envCfg env) (envTok env) genesisAddr "rt-over" (OpUpdate leafActive leafTerminal) now
...
runOverJourney env recOver = do
    ...
    -- Since #157 the key that ENDS a name is the one the record's own
    -- commitment opens to, not the control key the recovery installed
    -- (D-TERMINATE). The recovery rotated control to the destination
    -- and committed to this next key; this is the key that signs the
    -- retire, so this is the key the linkage pins.
    let destHash = addrKeyHashBytes freshRetireAddr
    ...
    emit
        ...
        ( "OV-recovery-linkage: retire "
            ...
            <> " under the committed recovery key 0x"
            <> hex destHash
...
runRows env recAccept2 recRefusals recDuplicates recR1 recR2 recOver = do
    ...
    -- LT02 accepts, consuming accept2 into custody. It is the only
    -- accepting retirement left: LT01 retired with the route it stood
    -- for — the control key alone no longer ends a name (D-TERMINATE) —
    -- and the withdrawal-witness row went with the consumer hook the
    -- fold no longer invokes. The journey README maps both.
    snapA2 <- mustSnap env recAccept2
    signedLT02 <- rowLT02 env snapA2
    -- LT09 replays LT02's exact transaction against the consumed record.
    rowLT09 env recAccept2 signedLT02
    ...
    -- Mismatched pair (NOTE-029 N2): LT02 custody with the RR1
    -- request must refuse (wrong pairing, each piece genuine).
    rowN2Mismatch env signedLT02 signedRR1
...
rowLT02 env snap = do
    tx <- retireTx env snap (envCustodyAddr env) [envQuorum1Hash env, envQuorum2Hash env] "rt-accept2" BS.empty
    let signed =
            addKeyWitness
...
rowRetireCtl env snap = do
    tx <- retireTx env snap (envCustodyAddr env) [envQuorum1Hash env, envQuorum2Hash env] "rt-ctl" BS.empty
    let signed =
            addKeyWitness
                (mkSignKey quorum1Seed)
                (addKeyWitness (mkSignKey quorum2Seed) (addKeyWitness genesisSignKey tx))
...
rowRecoverRecord env snap label = do
    ...
    let (destAddr, destHash) = recoveryDest
        -- The successor commits to a next control key that actually
        -- exists. Since #157 ending a name takes the proof recovering
        -- it takes (D-TERMINATE), so a record whose commitment nobody
        -- can open has only the quorum route left — and RR1 below is
        -- the row that shows the recovery route surviving a rotation.
        freshCommitment = nextControlCommitmentOf (serialiseAddr freshRetireAddr)
...
{- | The recovery route across a rotation: the key the recovered
record's own commitment opens to is revealed and signs, and it alone
ends the name. Since #157 that is the route — the CURRENT control key
does not end a name, only the committed recovery key or the quorum
does (D-TERMINATE), and this row is what shows the recovery half of
that surviving a rotation. Reuses custody/gone assertions for RR1 and
the Over journey.
-}
rowRetireRecoveredController :: Env -> Snap -> String -> ByteString -> IO ConwayTx
rowRetireRecoveredController env snap label key = do
    let freshHash = addrKeyHashBytes freshRetireAddr
    tx <-
        retireTx
            env
            snap
            ...
            [freshHash]
            key
            (serialiseAddr freshRetireAddr)
    let signed = addKeyWitness (mkSignKey freshRetireSeed) (addKeyWitness genesisSignKey tx)
    unless
        (Set.singleton (addrWitnessKeyHash freshHash) == (signed ^. bodyTxL . reqSignerHashesTxBodyL))
        $ failWith (label <> ": required signers must be exactly the revealed recovery key")
    when
        ( any
            (`Set.member` (signed ^. bodyTxL . reqSignerHashesTxBodyL))
            [addrWitnessKeyHash (envOldHash env), addrWitnessKeyHash (envQuorum1Hash env), addrWitnessKeyHash (envQuorum2Hash env)]
    ...
    emit
        "row"
        ( label
            <> "-recovery-then-committed-key-retire-accepts: accepted tx="
            <> txIdHex signed
            <> " key=0x"
            <> hex key
            <> " (the key survives rotation; the committed recovery key \
               \alone ends the name, old controller and quorum absent; the \
               \active token is at custody)"
        )
    pure signed

{- | The next control key a recovery in this runner commits to, and the
one a retirement through the recovery route then reveals.
-}
freshRetireSeed :: ByteString
freshRetireSeed = "s66-fresh-retire0000000000000000"

freshRetireAddr :: Addr
freshRetireAddr = enterpriseAddr (keyHashFromSignKey (mkSignKey freshRetireSeed))
...
rowRetireRecoveredQuorum env snap = do
    tx <- retireTx env snap (envCustodyAddr env) [envQuorum1Hash env, envQuorum2Hash env] "rt-rr2" BS.empty
    let signed =
            addKeyWitness
                (mkSignKey quorum1Seed)
                (addKeyWitness (mkSignKey quorum2Seed) (addKeyWitness genesisSignKey tx))
...
rowLT03 mode env snap = do
    tx <- retireTx env snap (envCustodyAddr env) [envQuorum1Hash env] "rt-refusals" BS.empty
...
rowLT03Dup env snap = do
    tx <- retireTx env snap (envCustodyAddr env) [envQuorum1Hash env] "rt-duplicates" BS.empty
    let signed = addKeyWitness (mkSignKey quorum1Seed) (addKeyWitness genesisSignKey tx)
    expectRefused
        MainRun
        env
...
rowLT08 :: Env -> Snap -> IO ()
rowLT08 env snap = do
    -- Authorized by the route the contract accepts — the quorum, since
    -- #157 makes the control key alone no route at all — so the only
    -- thing wrong with this attempt is where the active token goes.
    tx <-
        retireTx
            env
            snap
    ...
    expectRefused
        ...
        "retirement-request"
        ( "authorized by the quorum but the active token goes anywhere \
...
rowLT09 env consumedIn signedLT02 = do
    ...
    emit
        ...
        ( "LT09-retirement-replay-refused: retiring "
            <> showIn consumedIn
            <> " again with LT02's exact tx="
            <> txIdHex signedLT02
            <> " (the record is gone)"
        )
    result <- submitTx (envSubmit env) signedLT02
    case result of
        Submitted _ ->
            failWith
                ...
                \retirement twice"
        Rejected reason -> do
            tag <- retainTx env "LT09-replay" signedLT02
...
retirement (the updateTerminal completion the route-authorized
retire co-created). It sits at the ordinary request validator.
-}
...
{- | Mismatched pair (NOTE-029 N2): LT02's live custody with RR1's
pending completion request — each piece genuine (live custody, a
real pending termination, an exact burn of the custody-held token),
but the pairing is wrong: different creator transactions, and
therefore different keys. Must refuse at local evaluation naming a
script, spending nothing and losing no collateral.
...
{- | Book an activation for a key and build the fold of it (LX01
shape): returns the build outcome (`Left` = local-evaluation refusal).
Shared by the MainRun row and both control modes.
-}
refoldAttempt :: Env -> ByteString -> IO (Either SomeException ConwayTx)
refoldAttempt env key = lx01Attempt env key

rowN2Mismatch :: Env -> ConwayTx -> ConwayTx -> IO ()
rowN2Mismatch env signedLT02 signedRR1 = do
    custodyLT02 <- overCustodyOut env signedLT02 "N2-lt02-custody"
    reqRR1 <- overRequestOut env signedRR1 "N2-rr1-request"
    feeUtxo <- queryRetirementFee env
    foldResult <-
        try (buildMismatchFold env custodyLT02 reqRR1 feeUtxo genesisAddr) :: IO (Either SomeException (ConwayTx, Root))
    case foldResult of
        Left err -> do
            -- Strict attribution (NOTE-030): named script-hash field,
            -- semantic (never budget), at a spend purpose. Since #157
            -- the script that meets this pairing first is the cage's
            -- own: the request names key rt-rr1, whose termination owes
            -- a burn of THAT key's active token, and what the custody
            -- offers is rt-accept2's. The delta is the registry's own
            -- arithmetic, so the registry refuses it.
            pinEvalRefusal
                "N2"
                (show err)
                (hex (scriptHashBytes (cfgScriptHash (envCfg env))))
                "ConwaySpending"
            emit
                "row"
                ( "N2-mismatched-pair-refused: REFUSED at build — the fold pairs \
                  \LT02 custody with the RR1 request (different creators, \
                  \different keys): the registry state script refuses the \
                  \pairing, because the token the terminating edge owes is not \
                  \the token the custody holds: "
                    <> show err
                )
        Right _ ->
...
{- | Book the activation of @key@ and fold it. This is the whole of a
re-registration since #157: an approval certifying `insertActive`, the
request that carries it, and a fold. The booking lands — certifying an
edge says nothing about whether the trie can take it — and the fold is
where an occupied key is refused, by the registry's own state script.
-}
lx01Attempt :: Env -> ByteString -> IO (Either SomeException ConwayTx)
lx01Attempt env key = do
    reqIn <- bookActivation env key ("LX01-" <> show key)
    emit
        "row"
        ( "LX01-re-registration-queued: activation of "
            <> show key
            <> " booked "
            <> showIn reqIn
            <> " (booking is not the refusal; the fold is)"
        )
    try (foldPending env) :: IO (Either SomeException ConwayTx)

-- | Book one @insertActive@ of @key@ against this run's registry.
bookActivation :: Env -> ByteString -> String -> IO TxIn
bookActivation env key label = do
    let cfg = envCfg env
        datumData = namingDataToData (envDatum env)
        destHash =
    ...
    unsigned <-
        bookEdgeTx
            ...
            key
            (OpInsert leafActive)
            (serialiseAddr (envAppAddr env), destHash)
            []
            recordBond
    let signed = addKeyWitness genesisSignKey unsigned
    submitAccepted env label signed
    ...
    mustFindUTxO (envProv env) reqAddr (txIdHex signed) "registry request"

-- | Fold whatever the registry has pending, duties discharged.
foldPending :: Env -> IO ConwayTx
foldPending env = do
    let cfg = envCfg env
        datumData = namingDataToData (envDatum env)
        destHash =
...
rowLX01 env = do
    ...
    foldResult <- lx01Attempt env "rt-over"
    case foldResult of
        Left err -> do
...
runControlValid env recRefusals = do
    ...
    decoyReq <- submitRetirementRequest env "rt-n2-decoy" leafActive
    ...
    tx <- retireTx env snap (envCustodyAddr env) [envQuorum1Hash env, envQuorum2Hash env] "rt-refusals" BS.empty
    let signed =
            addKeyWitness
                (mkSignKey quorum1Seed)
                (addKeyWitness (mkSignKey quorum2Seed) (addKeyWitness genesisSignKey tx))
...
runControlWrongReason env recDuplicates recRefusals = do
    -- A real retirement first (accepts, emits a row, proving the runner
    -- ran). It is the quorum route: since #157 the control key alone is
    -- no route, so the accepting retirement LT01 used to be is gone.
    snapDup <- mustSnap env recDuplicates
    signedA1 <- rowRetireCtl env snapDup
    -- Eval-branch controls (NOTE-033): mismatched pair and occupied
    -- refold through the impossible marker (fired + recorded here;
    -- keys are this runner's own setup conventions).
    decoyReq <- submitRetirementRequest env "rt-n2-decoy" leafActive
    requireFired "N2-CWR" =<< rowN2ControlWrongReason env signedA1 decoyReq
    requireFired "LX01-CWR" =<< rowLX01ControlWrongReason env "rt-duplicates"
...
retireTx ::
    ...
    ByteString ->
    {- | the revealed recovery key, or empty bytes when the quorum is
    the route instead
    -}
    ByteString ->
    IO ConwayTx
retireTx env snap destination signers key revealed = do
    unless (snapRepresentative snap == key) $
        failWith "retire: the record does not carry the active token of this key"
    ...
    now <- currentPosixMs
    -- #157 F-003/N4: ending a name is a tree edge. The same transaction
    -- books the `updateTerminal` request the registry must fold — live
    -- `0x01` to over `0x02` for this key — and mints the approval that
    -- certifies it, bound to the request's own owner and naming no
    -- destination, because a termination delivers to nobody.
    -- The completion request sits at the ordinary request validator
    -- for this registry, not the cage/state address.
    let reqAddr =
            requestAddrFromCfg (envCfg env) (envTok env) Testnet
        owner = addrKeyHashBytes genesisAddr
        completionName = approvalName 3 key owner (BS.empty, BS.empty)
        completionTokens =
            Map.singleton
                (envAppPolicy env)
                (Map.singleton (AssetName (SBS.toShort completionName)) 1)
        reqDatum =
            mkRequestDatum
                (envTok env)
                genesisAddr
                key
                (OpUpdate leafActive leafTerminal)
                1_000_000
                now
        reqDraftOut =
            mkBasicTxOut reqAddr (MaryValue (Coin 0) (MultiAsset completionTokens))
        ...
        requestOut =
            mkBasicTxOut reqAddr (MaryValue (Coin reqCoin) (MultiAsset completionTokens))
                & datumTxOutL .~ mkInlineDatum reqDatum
        inputs = Set.fromList [snapIn snap, fst fund]
        spendIdx = spendingIndex (snapIn snap) inputs
        mintMA = MultiAsset completionTokens
        redeemers =
            Redeemers
                ( Map.fromList
                    [
                        ...
                        , -- Generous (NOTE-023 class): the retire carries the
                          ...
                          (Data (redeemerRetire key revealed), generousUnits)
                        )
                    ,
...
constructor 2 of the #157 application redeemer. The representative list
it used to carry is gone with the representative: the token a record
holds is the registry's active witness, and `recover` reads it off the
record's own value.
-}
...
recoverTx ::
    ...
    [ByteString] ->
    IO ConwayTx
recoverTx env snap revealed registry successor signers = do
    (fund, collateral) <- takeFundCollateral env
    let inputs = Set.fromList [snapIn snap, fst fund]
        spendIdx = spendingIndex (snapIn snap) inputs
        redeemers =
...
bootRetirementCage seedRef prov submit tm stateBytes requestBytes appPin activePin absentPin terminalPin evDir evNext = do
    let cfg =
            CageConfig
...
The claim-and-approval shape this replaced is retired with the
mechanism it described: there is no pending claim for a fold to consume
and no representative token. What the rows below assert about a record
is unchanged; only how one comes to exist is.
-}
setupRecoveryRecord ::
    Env ->
    NamingDatum ->
    ...
    ByteString ->
    IO (String, TxIn)
setupRecoveryRecord env datum label key = do
    let cfg = envCfg env
        prov = envProv env
...
{- | Spend redeemer @Retire { key, revealed_control }@ — constructor 1 of
the #157 application redeemer (Maintain 0, Retire 1, Recover 2). The
claim-and-cancel arms it used to sit behind are retired.

The reveal rides the redeemer because ending a name needs exactly the
proof recovering it needs: the preimage of the record's committed next
control. Empty bytes when the quorum is the route instead.
-}
redeemerRetire :: ByteString -> ByteString -> PLC.Data
redeemerRetire key revealed =
    PLC.Constr 1 [PLC.B key, PLC.B revealed]

{- | Mint redeemer @Approve { edge, key, owner, destination }@ — the
application policy's only mint redeemer since #157 N2.
-}
approveRedeemer ::
    Integer ->
    ByteString ->
...
checkPinnedRepresentative unappliedHex = do
    ...
    let pins =
            [ mpHash p
            | p <- manifestValidators manifest
            , "witness.witness.mint" `T.isPrefixOf` mpTitle p
            ]
    unless (length pins >= 1) $
        failWith
            "identity: no witness.witness.mint pin in the \
            \manifest"
...
{- | The asset a retirement moves to custody, read from its own body:
the completion request it co-created names the key, and since #157
(D-ASSET) the asset name under each token policy IS the registry key —
the hashed representative name is gone.
-}
retirementRepresentative :: ConwayTx -> ByteString
retirementRepresentative tx = case [ requestKey req
