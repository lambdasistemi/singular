    ...
    if null missingRows && okFields == totalFields && null extraRows && null resurrected
        then do
            putStrLn
                ( "no drift: "
                    ++ show okFields
                    ++ " comparisons identical across "
                    ++ show (length pinnedVecIds)
                    ++ " pinned vectors ("
                    ++ show (length retiredVecIds)
            ...
            putStrLn
                ( "drift: the live corpus has moved relative to the vendored v0.2.0 pin "
                    ++ "("
                    ++ show (totalFields - okFields)
                    ++ " divergent field(s), "
                    ++ show (length missingRows)
                    ++ " missing row(s), "
                    ++ show (length extraRows)
                    ++ " extra row(s)); the pin stays - "
