...
submission time, the transaction's validity bounds, its signatories) and refused
`withdraw-insert-only`, `retract-owner` or `not-phase2` before anything it spends
or pays is judged; a retraction with no witness is `unsupported`. A fold and a
reject are exactly as before. On chain the witness is read from the retraction as
built. Rows restated: the paragraph above the table, `retract`, `tx`, `mint`,
`settle`, surface digest and outcome class.
Consumers: `lean/driver-corpus.json` (one accepted retraction and one refused per
admission reason), `conformance/lean/DriverTransport.lean` (a retraction question
carries its witness, no other question may), `Conformance.Run.Live` (the model
...
`Singular.admittedExitStep` at the request the scenario carries: a fold of one of
the seven edges, a reject, or a retract. A retract is admitted first, by
`Singular.retractAdmission` under the witness its scenario carries; a fold and a
reject have no admission and are exactly `Singular.exitStep`
(`Singular.Statements.admitted_exit_is_the_exit`). A fold is `Singular.step`
itself, and is refused `exit-edge-mismatch` when the request names another edge.
An operation is refused when `Singular.retractAdmission`, `Singular.refusal` or
`Singular.exitStep` returns a reason; the driver reports that reason verbatim and
never manufactures one. The driver also declares two judgements, `spend` then
`settle`, answered about a transaction a caller observed rather than one the
model built, after a retraction's admission has been answered.
...
| `mint` | realization | the executed result's own `mint`: for a fold, the R2 keyed delta of the edge, each entry named by the registry's pinned policy for its kind and the asset name the model derives from the key; for a reject and an admitted retraction, empty, since neither folds an edge. |
...
| surface digest | identity | `definitionDigest` is taken over the declared operation, observation, unobservable and judgement names together, so a silently widened or narrowed surface changes it. A scenario's fields, a retraction's witness among them, are not part of the surface and do not move it. |
...
| outcome class | identity | `accepted`, `refused` and `unsupported` are disjoint. Only `accepted` carries observations; `refused` carries a reason `Singular.retractAdmission`, `Singular.refusal` or `Singular.exitStep` can produce, and a retraction only one of admission's; `unsupported` is the driver failing to reach the case — a retraction with no witness among them — and is never reported as a ledger refusal. |
