...
build_out=$(mktemp)
refusal_out=$(mktemp)
trap 'rm -f "$json_out" "$build_out" "$refusal_out"' EXIT
...
driver_bin=$(cd "$offchain" && nix develop --command cabal list-bin exe:retirement-rows)
check_bin=$(cd "$offchain" && nix develop --command cabal list-bin exe:rival-driver-predevnet-check)
test -x "$driver_bin" || { echo "PREDEVNET RED: driver binary not executable: $driver_bin" >&2; exit 1; }
...
for offline_value in absent 0; do
  ...
  rg -Fxq 'RIVAL_OFFLINE_ONLY=1 required' "$refusal_out" || {
