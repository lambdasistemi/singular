...
def ABC_HASH : ByteString :=
  bs [
      0xBD, 0xDD, 0x81, 0x3C, 0x63, 0x42, 0x39, 0x72, 0x31, 0x71, 0xEF, 0x3F,
      0xEE, 0x98, 0x57, 0x9B, 0x94, 0x96, 0x4E, 0x3B, 0xB1, 0xCB, 0x3E, 0x42,
...
/-- Retained mistranscription (hand-transcribed 2026-09-12, caught the same day
by owner cross-check against python `hashlib`; see EVIDENCE.md F-002). Kept as a
known-bad vector: the evaluator must NOT produce these bytes. Guards against
regression toward the wrong value, never an expectation of it. -/
def WRONG_HASH : ByteString :=
  bs [
      0xBD, 0xDD, 0x81, 0x3C, 0x63, 0x42, 0x39, 0x72, 0x31, 0x71, 0xEF, 0x3F,
      0xEE, 0x98, 0x56, 0x79, 0x94, 0x96, 0x4E, 0x3B, 0xBB, 0x1C, 0xB3, 0xE4,
      0x27, 0x62, 0x2C, 0x8C, 0x06, 0xD5, 0x23, 0x19]

def wrongRefuted : Bool :=
  match hashBytes (cekExecuteProgramWithSemanticVariant targetSemVar
      (hashProg BuiltinFun.Blake2b_256 "abc") [] 1000) with
  | some h => h != WRONG_HASH
  | none => false
