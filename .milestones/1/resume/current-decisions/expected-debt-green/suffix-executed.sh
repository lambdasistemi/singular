...
grep -q 'complete: 13/13 rows ok' /tmp/singular-expected-debt-green-7pjglt/run.log || { echo 'FAIL: the session did not complete all 13 rows (mid-run failure or crash)'; exit 1; }
#    ...and the terminal failure is the held-rows debt report
grep -q 'ROWS THE RUN CANNOT REPORT AS PASSING' /tmp/singular-expected-debt-green-7pjglt/run.log || { echo 'FAIL: the terminal failure is not the held-rows debt report'; exit 1; }
# 3. exactly the thirteen rows plus the session CL01, no extras
printf '%s\n' CG02 CG03 CG04 CG05 CG07 CG09 CG10 CG11 CG12 CG13 CG17 CG19 CG20 CL01 | sort > /tmp/singular-expected-debt-green-7pjglt/expected-rows.txt
/run/current-system/sw/bin/jq -r '.row' ./conformance-receipts/receipt-*.json | sort > /tmp/singular-expected-debt-green-7pjglt/receipt-rows.txt
diff /tmp/singular-expected-debt-green-7pjglt/expected-rows.txt /tmp/singular-expected-debt-green-7pjglt/receipt-rows.txt || { echo 'FAIL: receipt set mismatch (missing or extra row)'; exit 1; }
...
for row in CG02 CG03 CG04 CG05 CG07 CG09 CG10 CG11 CG12 CG13 CG17 CG19 CG20 CL01; do
  ...
  v="$(/run/current-system/sw/bin/jq -r '.verdict' "$f")"
  want="$(expect_verdict "$row")"
  [ "$v" = "$want" ] || { echo "FAIL: unexpected verdict for $row: got '$v', expected '$want'"; exit 1; }
  [ "$(/run/current-system/sw/bin/jq -r '.dirty' "$f")" = "false" ] || { echo "FAIL: $row receipt records a dirty tree"; exit 1; }
  [ "$(/run/current-system/sw/bin/jq -r '.base' "$f")" = "$head_sha" ] || { echo "FAIL: $row receipt is not bound to this candidate ($head_sha)"; exit 1; }
done
# 5. CL01 carries the session's measurement evidence: nonzero
#    units and size, folds named — refuse missing or divergent
/run/current-system/sw/bin/jq -e '.mem > 0 and .cpu > 0 and .txSize > 0 and (.transactions | length > 0)' ./conformance-receipts/receipt-CL01.json > /dev/null || { echo 'FAIL: CL01 measurement evidence missing or divergent'; exit 1; }
# 6. the run's own accounting must name exactly the expected
#    held set, with nothing failing against this candidate
held="$(sed -n 's/^- Held .*held-q002): //p' /tmp/singular-expected-debt-green-7pjglt/run.log)"
...
failing="$(sed -n 's/^- Failing .*accepted): //p' /tmp/singular-expected-debt-green-7pjglt/run.log)"
