...
printf '%s\n' CG02 CG03 CG04 CG05 CG07 CG09 CG10 CG11 CG12 CG13 CG17 CG19 CG20 CL01 | sort > /tmp/singular-expected-debt-control-1F5pLb/expected-rows.txt
/run/current-system/sw/bin/jq -r '.row' ./conformance-receipts/receipt-*.json | sort > /tmp/singular-expected-debt-control-1F5pLb/receipt-rows.txt
diff /tmp/singular-expected-debt-control-1F5pLb/expected-rows.txt /tmp/singular-expected-debt-control-1F5pLb/receipt-rows.txt || { echo 'FAIL: receipt set mismatch (missing or extra row)'; exit 1; }
...
held="$(sed -n 's/^- Held .*held-q002): //p' /tmp/singular-expected-debt-control-1F5pLb/generic-rows.log)"
...
failing="$(sed -n 's/^- Failing .*accepted): //p' /tmp/singular-expected-debt-control-1F5pLb/generic-rows.log)"
