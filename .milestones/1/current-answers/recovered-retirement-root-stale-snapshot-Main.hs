...
-- (control oldAddr -> destAddr), asserting rotation plus payment and
-- quorum preservation from the chain datum. Returns the rotated snap.
rowRecoverRecord :: Env -> Snap -> String -> IO Snap
rowRecoverRecord env snap label = do
    current <- chainDatumOf env snap label
    ...
    rotated <- chainDatumOf env snap (label <> "-rotated")
    ...
    mustSnap env (TxIn (txIdTx signed) (TxIx 0))
