...
ARTIFACT_PURPOSE_KIND = {
    ...
    "producer-application": "MintingScript",
    "producer-applied-representative": "MintingScript",
...
def inspect_ledger_semantics(
    ...
    for rid, row in rows.items():
        exact_keys(
            row,
            {"fields", "witnesses", "created_output_indices", "created_output_assertions"},
...
def validate_ledger_plan(plan: dict[str, Any], seal: dict[str, Any], raw: Path) -> dict[str, Any]:
    ...
    for n, row in enumerate(plan["rows"]):
        ...
        if not isinstance(row["watched_txins"], list) or not row["watched_txins"] or not all(
            isinstance(x, str) and re.fullmatch(r"[0-9a-f]{64}#[0-9]+", x)
            for x in row["watched_txins"]
        ):
            fail(f"{rid}: watched_txins invalid")
        if len(row["watched_txins"]) != len(set(row["watched_txins"])):
            fail(f"{rid}: watched_txins contains duplicates")
        if not isinstance(row["watched_addresses"], list) or not row["watched_addresses"] or not all(
            isinstance(x, str) and x for x in row["watched_addresses"]
        ):
            fail(f"{rid}: watched_addresses invalid")
        if len(row["watched_addresses"]) != len(set(row["watched_addresses"])):
            fail(f"{rid}: watched_addresses contains duplicates")
...
def run_ledger_rows(
    *,
    ...
    documents = {rid: {"view": views[rid], "before": before[rid]} for rid in LEDGER_ROWS}
