...
runRows env recAccept1 recAccept2 recRefusals recDuplicates = do
    ...
    -- RIVAL WITNESS (issue #80 t80e): install B's state as the ONLY anchor
...
-- ---------------------------------------------------------
-- Rival-cage witness (issue #80 t80e; NOTE-017/019 bounds)
-- ---------------------------------------------------------

data RivalVariant = RivalOwnPolicy | RivalCopiedPolicy | RivalForgedAnchor
    deriving stock (Show, Eq)

readRivalVariant :: IO (Maybe RivalVariant)
readRivalVariant =
    lookupEnv "RIVAL_VARIANT" >>= \m -> pure $ case m of
        Nothing -> Nothing
        Just "own-policy" -> Just RivalOwnPolicy
        Just "copied-policy" -> Just RivalCopiedPolicy
        Just "forged-anchor" -> Just RivalForgedAnchor
        Just other -> error ("RIVAL_VARIANT: unknown variant " <> other)
...
-- representative_policy per the variant) and install B's state as the
-- retirement anchor. The forged-anchor variant anchors on a tokenless
-- output at the state address instead (creating an output never runs the
-- script), labelled separately.
  ...
  where
    ...
    bootAndAnchor rivalRef bPolicy = do
        ...
        bState <- case findStateUtxo (cagePolicyIdFromCfg cfgB) tokB utxos of
            Just x -> pure x
            Nothing -> failWith "rival B: state UTxO not found after boot"
    ...
    forgedAnchor rivalRef = do
        ...
        -- never executes the script (the CA05 observation), so this
        -- input is constructible by anyone and carries NO cage token.
    ...
    mintlessOutputAtState = do
        ...
        let stateAddr = cageAddrFromCfg (envCfg env) Testnet
            ...
            body = mkBasicTxBody & outputsTxBodyL .~ StrictSeq.singleton out
            tx = mkBasicTx body
        balanced <- evaluateAndBalance (envProv env) pp [feeUtxo] genesisAddr tx
        let signed = addKeyWitness genesisSignKey balanced
            outRef = TxIn (txIdTx signed) (TxIx 0)
        _ <- submitWithRetirementGenesis (envSubmit env) signed
        -- read the created output back from the chain (the settled form)
        created <- Cage.queryUTxOs (envProv env) stateAddr
        case [ (i, o) | (i, o) <- created, i == outRef ] of
            (pair : _) -> pure pair
...
queryRetirementState env = do
    rival <- readIORef (envRivalAnchor env)
    case rival of
...
setupRecoveryRecord env datum label spelling = do
    ...
    -- Rival witness: install B's state as the anchor ONLY now — A's records
    -- were created under A; the fold that retires the name resolves B.
