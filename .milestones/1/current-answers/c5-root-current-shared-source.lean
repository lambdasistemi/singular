...
-- Diagnostic leg (mint purpose, isolated +1 context): the insert-approval
-- derivation through the real bytes, with no state/record/continuation
-- involved. HALT proves the preimage (domain || 0x00 || control ||
-- commitment); ERROR convicts it. Diagnostic only — not the span burn.
/-- info: "HALT" -/
#guard_msgs in
...
-- The exact composed transaction value shared by the Fold runner and the
-- mint-burn runner (NOTE-024): one definition, two purpose observations.
-- Both legs execute against this identical TxInfo — same inputs, outputs,
-- complete mint and redeemer map — so the burn half and the fold half are
-- observations of one transaction shape, not two lookalikes. Any edit
-- here moves both legs together by construction.
def spanTxComposed : TxInfo := spanTxInfo DDFC_REP DDFC_REP
...
def runSpanStateComposed (steps : Nat) : State :=
  cekExecuteProgramWithSemanticVariant targetSemVar ddfcStateSpend.script
    [toTerm (⟨spanTxComposed, spanModifyRedeemer,
       .SpendingScript stateRef (some (ddfcDatum ROOT_ZERO DDFC_REP))⟩
       : ScriptContext)] steps

def runSpanRepComposed (steps : Nat) : State :=
  cekExecuteProgramWithSemanticVariant targetSemVar ddfcRepApplied.script
    [toTerm (⟨spanTxComposed, spanMintRedeemer,
       .MintingScript DDFC_REP⟩ : ScriptContext)] steps

-- NOTE-025: the state, representative and application-spend observations
-- share one exact TxInfo with the mint-burn observation below — four
-- runners, one value. Parameterized runners above stay for
-- negative/discrimination legs only.

def runSpanAppComposed (steps : Nat) : State :=
  cekExecuteProgramWithSemanticVariant targetSemVar ddfcAppSpend.script
    [toTerm (⟨spanTxComposed, spanFoldData,
       .SpendingScript claimRef (some spanRecord)⟩ : ScriptContext)] steps

def runSpanAppMintBurn (steps : Nat) : State :=
  cekExecuteProgramWithSemanticVariant targetSemVar ddfcAppSpend.script
    [toTerm (⟨spanTxComposed, spanInsertApprovalRedeemer,
       .MintingScript DDFC_APP⟩ : ScriptContext)] steps

-- C5 burn leg (NOTE-022, corrected NOTE-024): the application's actual
-- MINT purpose executed for the span's -1 burn, in the EXACT composed
-- transaction value shared above — same inputs, outputs, complete mint
-- (rep +1 and approval -1) and redeemer map as the Fold observation.
-- The `burn_rides_claim` binding ties it to the identical live claim
-- the Fold consumes. HALT observes the burn half of the composed shape.
/-- info: "HALT" -/
#guard_msgs in
#eval outcome (runSpanAppMintBurn 200000)
...
/-- info: ("HALT", "HALT", "HALT") -/
#guard_msgs in
#eval (outcome (runSpanStateComposed 200000),
       outcome (runSpanRepComposed 200000),
       outcome (runSpanAppComposed 200000))
...
/-- info: ("HALT", "HALT", "HALT") -/
#guard_msgs in
#eval (outcome (runSpanAppComposed 500000),
       outcome (runSpanAppComposed 1000000),
       outcome (runSpanAppComposed 2000000))
