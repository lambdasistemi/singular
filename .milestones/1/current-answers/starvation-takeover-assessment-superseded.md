# Starvation / takeover assessment — SUPERSEDED 2026-09-12T19:25Z

> **The commissioned trace is withdrawn and must not be run.** The stakeholder accepts SPO selection
> as an **operating assumption** — *"I wouldn't trip on that SPOs have power, we hope they care more
> about making money with the batches"*. **Starvation resistance, guaranteed takeover and inclusion
> are not M1 acceptance requirements.**
>
> This document is **retained as bounded diagnostic evidence**, not deleted. Its model mapping stays
> accurate and was never the disputed part.
>
> **Waived: nothing else.** Promised payment correctness stands, and the model's request timing,
> refund and recovery rules stand — `inPhase1` / `inPhase2` / `rejectable` + `userPostable`
> remain exactly as binding. This ruling concerns **who may include a transaction**, not **what the
> validator must enforce when one is included**. It is an accepted assumption backed by batch
> earnings as incentive, **not** a proven fairness or equilibrium guarantee, and **not** permission
> to change fees.
