...
import RivalMintlessBody (mkMintlessAnchorBody)
...
import Cardano.Ledger.Core (Script, eraProtVerLow, extractHash)
...
runMode mode blueprintPath mpfsPath = do
    ...
    withCardanoNode gDir $ \sock _startMs -> do
        ...
        (cfg, tok, aBootTx) <- bootRetirementCage prov submit tm stateBytes requestBytes (SBS.toShort (scriptHashBytes repAppliedHash))
        writeEvidenceTxBody "a-boot-body.cbor" aBootTx
        createTrie tm tok
...
runRows env recAccept1 recAccept2 recRefusals recDuplicates = do
    ...
    case planMode mVariant of
        ...
        WitnessTerminal variant expectation -> do
            ...
            -- (NOTE-048.3) raw evidence: the exact signed target body, the
            -- attempted record/anchor inputs, and pre-submission UTxO
            -- observations.
            writeEvidenceTxBody "target-body.cbor" signed
            writeEvidenceText "attempted-inputs.txt"
                (unlines [showIn (snapIn snapA1), showIn bAnchorIn])
            writeEvidenceText "pre-utxos-app.txt"
                =<< (renderUtxos <$> Cage.queryUTxOs (envProv env) (envAppAddr env))
            writeEvidenceText "pre-utxos-state.txt"
                =<< (renderUtxos <$> Cage.queryUTxOs (envProv env) anchorAddr)
            writeEvidenceText "pre-utxos-genesis.txt"
                =<< (renderUtxos <$> Cage.queryUTxOs (envProv env) genesisAddr)
            ...
            -- (NOTE-048.3) the raw typed submission result and
            -- post-submission UTxO observations (refusal liveness and
            -- successor recovery are recoverable from these).
            writeEvidenceText "target-submission-result.txt" (show result)
            writeEvidenceText "post-utxos-app.txt"
                =<< (renderUtxos <$> Cage.queryUTxOs (envProv env) (envAppAddr env))
            writeEvidenceText "post-utxos-state.txt"
                =<< (renderUtxos <$> Cage.queryUTxOs (envProv env) anchorAddr)
            writeEvidenceText "post-utxos-genesis.txt"
                =<< (renderUtxos <$> Cage.queryUTxOs (envProv env) genesisAddr)
            case result of
                Submitted _ ->
                    case expectation of
                            ...
                            case (mRoot, succs) of
                                (Just (Root want), [(succIn, succOut)]) -> do
                                    ...
                                    case checkCopiedSuccessor facts of
                                        Right () -> do
                                            bSeedNow <- readIORef (envRivalBSeed env)
                                            writeEvidenceText "b-successor-facts.txt"
                                                ( "facts=" <> show facts
                                                    <> "\nconfigured-seed=" <> maybe "none" showIn bSeedNow
                                                    <> "\nderived-token=" <> show tokB
                                                    <> "\nstate-address=" <> show stateAddr
                                                )
...
bootRetirementCage ::
    ...
    SBS.ShortByteString ->
    SBS.ShortByteString ->
    IO (CageConfig, TokenId, ConwayTx)
bootRetirementCage prov submit tm stateBytes requestBytes repPolicy = do
    ...
    -- (NOTE-048.3) raw evidence: the exact signed boot body, its inputs,
    -- and the typed submission result.
    writeEvidenceTxBody "boot-body.cbor" signedBoot
    writeEvidenceText "boot-submission-result.txt" (show result)
    writeEvidenceText "boot-inputs.txt" (unlines (map showIn (Set.toList (signedBoot ^. bodyTxL . inputsTxBodyL))))
    pure (cfg, tok, signedBoot)
...
-- (NOTE-048.3) raw-evidence retention: exact bytes and text under the
-- per-variant TMPDIR (pinned by the official command). A source assertion
-- or summary line is not a substitute for the exact bytes.
evidenceFile :: FilePath -> IO FilePath
evidenceFile name = do
    m <- lookupEnv "TMPDIR"
    case m of
        Just tmp -> pure (tmp ++ "/rival-" ++ name)
        Nothing -> failWith "evidence retention: TMPDIR is not set"

writeEvidenceBytes :: FilePath -> ByteString -> IO ()
writeEvidenceBytes name bytes = do
    path <- evidenceFile name
    BS.writeFile path bytes
    hexPath <- evidenceFile (name ++ ".hex")
    writeFile hexPath (hex bytes)

writeEvidenceTxBody :: FilePath -> ConwayTx -> IO ()
writeEvidenceTxBody name tx =
    writeEvidenceBytes name (serialize' (eraProtVerLow @ConwayEra) (tx ^. bodyTxL))

writeEvidenceText :: FilePath -> String -> IO ()
writeEvidenceText name s = do
    path <- evidenceFile name
    writeFile path s

renderUtxos :: [(TxIn, TxOut ConwayEra)] -> String
renderUtxos utxos =
    unlines
        [ showIn i
            <> " coin=" <> show c
            <> " assets=" <> show (renderAssets o)
            <> " datum=" <> show (show <$> datumDataOf o)
        | (i, o) <- utxos
        , let Coin c = o ^. coinTxOutL
        ]
  where
    renderAssets o = case o ^. valueTxOutL of
        MaryValue _ (MultiAsset ma) ->
            [ (pid, hex (SBS.fromShort (assetNameBytes an)), q)
            | (pid, perPolicy) <- Map.toList ma
            , (an, q) <- Map.toList perPolicy
            ]
...
runRivalSetup env repAppliedPolicyBytes = do
    ...
    readRivalVariant >>= \case
        ...
        Just variant -> do
            ...
            -- (NOTE-048.1) spent observations: every consumed wallet input
            -- (the configured seed plus any funding the real boot spent).
            -- None is special-cased: consumed inputs are simply not live,
            -- so current-liveness funding selection excludes them; the
            -- seed identity itself was bound mechanically above from the
            -- signed boot transaction.
            let consumed = [i | (i, _) <- genesisBefore, i `notElem` map fst genesisAfter]
            writeEvidenceText "consumed-wallet-inputs.txt"
                (unlines (map showIn consumed))
  where
    prov = envProv env
    ...
    bootAndAnchor rivalRef bPolicy = do
        ...
        (cfgB, tokB, signedBoot) <-
            bootRetirementCage prov (envSubmit env) (envTrie env)
                (envStateBytes env)
                (envRequestBytes env)
                bPolicy
        let stateAddr = cageAddrFromCfg cfgB Testnet
            policyB = cagePolicyIdFromCfg cfgB
            configuredSeedRef = cageSeed cfgB
        utxos <- Cage.queryUTxOs prov stateAddr
        bState <- case findStateUtxo policyB tokB utxos of
            Just x -> pure x
            Nothing -> failWith "rival B: state UTxO not found"
        -- (NOTE-048.1) mechanical bindings on the real signed boot
        -- transaction: the configured seed must be an actual input; the
        -- mint must carry exactly the derived B token at quantity one; and
        -- B must be distinct from A's token.
        let bootInputs = signedBoot ^. bodyTxL . inputsTxBodyL
            bootSeedRefs = map txInToRef (Set.toList bootInputs)
        unless (configuredSeedRef `elem` bootSeedRefs) $
            failWith "rival boot: the configured cage seed is not an input of the signed boot transaction"
        let mintAssets = case signedBoot ^. bodyTxL . mintTxBodyL of
                MultiAsset ma -> Map.toList (Map.findWithDefault Map.empty policyB ma)
        unless (mintAssets == [(unTokenId tokB, 1)]) $
            failWith ("rival boot: the boot mint does not carry exactly the derived B token at quantity one: " <> show mintAssets)
        unless (tokB /= envTok env) $
            failWith "rival boot: B's derived token equals A's token"
        ...
        -- (NOTE-048.1) the seed identity is the configured cage seed,
        -- resolved to its exact TxIn on the signed boot transaction. Any
        -- additional consumed wallet inputs stay plain spent observations:
        -- they are not live, so current-liveness funding selection excludes
        -- them without any special case.
        configuredSeedTxIn <- case [i | i <- Set.toList bootInputs, txInToRef i == configuredSeedRef] of
            [i] -> pure i
            _ -> failWith "rival boot: the configured cage seed is not uniquely resolvable among the boot inputs"
        writeIORef (envRivalBSeed env) (Just configuredSeedTxIn)
        writeEvidenceText "boot-seed-binding.txt"
            ( "configured-seed=" <> showIn configuredSeedTxIn
                <> "\nboot-inputs="
                <> unlines (map showIn (Set.toList bootInputs))
            )
    ...
    mintlessOutputAtState = do
        ...
        feeUtxo <- case sortOn (Down . (^. coinTxOutL) . snd) utxos of
            ...
            (u : _) -> pure u
        let Coin avail = snd feeUtxo ^. coinTxOutL
            out :: TxOut ConwayEra
            out = mkBasicTxOut stateAddr (inject (Coin 2_000_000))
            -- (NOTE-048.2) the genuine script-free body: one seed input,
            -- one anchor output, exact-remainder fee, and NO
            -- script-integrity / protocol-parameters-view hash.
            body = mkMintlessAnchorBody (fst feeUtxo) (Coin avail) out
            signed = addKeyWitness genesisSignKey (mkBasicTx body)
        writeEvidenceTxBody "forged-anchor-body.cbor" signed
