...
exact output exhaustion, funding monotonicity, used monotonicity,
registry-derived conservation over `reqInputs + extraInputs ==
refunds + extraOutputs + fee`. `outs` names EXACTLY the refund stream
(no change outputs inside it); leftovers refuse as surplus. There is
deliberately NO caller input-total parameter. -/
def finish (preFund preUsed : List Nat) (fee extraInputs extraOutputs : Nat)
  ...
  | .ok (s', log, rem, paid, rest) =>
    ...
    | [] =>
      ...
      else if !(preUsed.all fun id => s'.base.used.contains id) then
        .error "used-dropped"
          ...
          if !hasDup (s.custody.map (fun b => b.requestId)) then
            if (s.custody.all fun b =>
              ...
              if !(items.all fun it =>
                ...
                .error "custody-spent"
              else finish (s.funding.map fun f => f.requestId) s.base.used
