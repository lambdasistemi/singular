...
## Correction 3 (NOTE-006, integrated-tree failure — root-caused and repaired)

The integrated-tree `lone_fork_insert` failure (ticket-77
`aiken-merge-clean.log`, clean `e9d08c5`, seed 178174350) was **not** a defect
in the patch, the vector, or the witness:

- **Witness binding proven by hashes**: the integrated tree's
  `onchain/validators/lone_fork.tests.ak` is byte-identical to this
  worktree's (sha256 `ab9dcf15dbef33116ce4ecdc7161f359642d5b6437d3e2eff30c8b37cf649d86`)
  and its literals match the retained producer packet exactly (`fbef6b6d…`,
  `cd1f3928…`, `651694f5…`, key `637330372d…`, value `7663`).
- **Staging proven unpatched on the failing invocation**: the
  `build/packages/aiken-lang-merkle-patricia-forestry` copy in the
  integrated worktree carries the defective v2.0.0 lone arm (no
  `bytearray.concat`) — aiken self-resolved it from the user cache. The
  failure signature (excluding mismatch; including and controls passing)
  is exactly the unpatched signature (`red2-unpatched-corrected.log`).
- **Mechanism**: dev-shell/vendor staging used `cp -r src dst`, which
  **nests** the copy when `dst` already exists — stale unpatched bytes
  survive silently.
- **Repair (commit `83b359f`, this worktree)**: `aikenPrelude` and the
  dev-shell hook now **remove-then-copy** the vendored packages, and both
  carry a mechanical staged-bytes guard (fixed arm must be present or
  build/shell entry fails). Verified: aiken-check/script-identity/
  regression green on the guarded staging; the guard fires on the
  unpatched v2.0.0 source; the dev-shell hook caught and then fixed the
  nested-staging case in this very worktree.
- `patches/README.md` now names the only valid reproduction paths (the Nix
  derivations). The ticket-77 lane re-verifies via
  `nix build .#checks.x86_64-linux.aiken-check` after re-integration; the
  stale `build/packages` in that worktree is regenerable and its
  replacement is that lane's operational step.

## Correction 2 (NOTE-005, 2026-09-12): occupied-key is builder-evaluation too; error retained; assertion strengthened

- The occupied-key negative is a **builder-evaluation refusal**
  (`updateTokenImpl` raised before submit — `updateToken: build failed:
  EvalFailure (ConwaySpending (AsIx 2))`, body `CekError`, script reason
  **opaque**, script `fa90391a…`). It is **not** a submitted ledger
  rejection. **Both negatives (occupied-key and patch-removal) are
  pre-submission builder evaluations.**
- The actual occupied ErrorCall is captured once, occupied-example only
  (`--match`), at `evidence/occupied-eval-error.log`; setup bound in the
  captured context: A, B, C already inserted and submitted, duplicate request
  (`cs07-fork-C11` / `vc2`) present, state root = committed {A,B,C} root.
- `Fork81Spec.hs` assertion strengthened (commit `f285637`): requires
  `EvalFailure` ∧ `ConwaySpending` ∧ `CekError` ∧ patched state script hash
  `fa90391a…` — the unpatched staging (`ce7615f6…`) cannot satisfy it.
- Final shared-candidate integration and consumer replay remain **not
  accepted**; this slice remains the owner-reviewed bounded scope.
