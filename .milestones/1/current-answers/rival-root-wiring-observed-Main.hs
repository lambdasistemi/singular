...
runRows env recAccept1 recAccept2 recRefusals recDuplicates = do
    ...
    -- boot installs B.
    (aStateIn, _aStateOut) <- queryRetirementState env
        ...
        WitnessTerminal variant expectation -> do
            ...
            case result of
                                    ...
                                    let valueAssets = case succOut ^. valueTxOutL of
                                        ...
                                        bTokenQty =
                                            ...
                                            of
                                                (q : _) -> q
                                                [] -> 0
                                        aPolicies = [envAppPolicy env, envRepPolicy env]
                                        ...
                                        facts =
                                            SuccessorFacts
                                                ...
                                                , sfATokenExcluded = not (any (\(pid, _, _) -> pid `elem` aPolicies) valueAssets)
                ...
                Rejected reason -> do
                    ...
                    emit "submit" ("raw submit result: " <> show result)
                    unless (isInfixOf "PlutusFailure" (show reason)) $
                        failWith "rival refusal: rejection lacks phase-2 evidence"
                    unless (isInfixOf (envAppHex env) (show reason)) $
                        failWith "rival refusal: the rejection does not name the exact application script hash"
