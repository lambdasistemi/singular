...
The settled [registry interface](registry-interface.md) is what this candidate is being brought to. Its state alphabet is the candidate's `Value` — active and over — with an absent state added and the terminal state named for what it is; its fold already refuses a mint that does not match the movements it folded, and its well-formedness invariant is already the active-token law. What the interface removes from the model is the incarnation counter, the asset scope, the identity-reuse flag and the pinned consumer script; what it adds is the absent token, the read, and any number of terminal tokens for a terminated key. The general whole-transition terminal-custody theorem this page names as a gap becomes one of the interface's stated guarantees. The Lean ticket that re-cuts the model is listed at the end of the interface page, and the pages derived from the model — the theorem inventory, the model ledger, the mutation proposals, the simulation and the clarity record — are re-derived by the same epic rather than edited by hand; until then they describe the candidate as proved.
