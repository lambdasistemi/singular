...
Build the documentation archive locally with `nix build .#docs-release`, and run version, workflow and artifact checks with `nix run .#release-check`. The archive contains the rendered site and speech data plus a review kit under `artifacts/`: the naming lifecycle contract, scenarios, replay sources, pinned toolchain inputs, and a byte-exact manifest that also covers the shipped model corpora, theorem inventories, and simulator identity. The inner `artifacts/SHA256SUMS` authenticates those review inputs; the outer `SHA256SUMS` authenticates the archive. Provenance comes from the validated tag and build binding. The release workflow has not been exercised against a real tag because no release is authorized yet.
