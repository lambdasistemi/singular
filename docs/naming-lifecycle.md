...
sequenceDiagram
  ...
  participant App as Naming application output
  participant Registry
  Anyone->>Registry: state Modify plus request Contribute for the spelling
  Anyone->>App: spend claim with Fold, naming the new representatives
  App->>App: burn insert approval, mint representative NFT
...
## Where the naming lifecycle is going

Nothing on this page that the holder does changes under the settled [registry interface](registry-interface.md): changing the destination and recovering control spend the record and never touch the registry, and retirement still moves the representative into completion-only custody before the fold burns it. What changes is the vocabulary and what becomes available afterwards. `Over` is the registry's terminal state; completion is the update to it. Once a name is terminal, anyone may submit a read of that state and receive a terminal token — the Over witness — that is true forever, may exist in any number, and can be burned freely. That token is what a contract uses to establish that a name has ended without reading the registry: the escrow in the naming demo pays the first name on its list whose representative is live and skips a name by presenting its Over witness. The interface page carries the contract; the naming demo epic carries the escrow.
