...
This is a **CANDIDATE at MODEL + PROOFS stage**. The executable Lean machine is `Singular.step` in [the model source](https://github.com/lambdasistemi/singular/blob/main/lean/Singular/Model.lean). Its finite corpus executes accepted and refused cases. Every theorem is PROVED from the standard axioms without changing its statement; there is no audit verdict or acceptance claim.
