...
class RealTreeDiscoveryTest(unittest.TestCase):
    """Current tree: 198 = 115 manifest-bound + 83 unclassified.

    #117 adds the connected withdrawal composition and pending maintenance
    examples to the previous 196. The protected base record stays unchanged.
    ...
    def test_current_population(self):
        ...
        self.assertEqual(len(decls), 198, "current population drifted; the denominator must be re-examined")
        statements = [d for d in decls if d.source.endswith("Statements.lean")]
        self.assertEqual(len(statements), 115)
