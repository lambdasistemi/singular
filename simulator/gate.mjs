...
for(const declaration of ledger){const short=declaration.name.split('.').at(-1),checker=checks[short];const rows=records.map(record=>({record,row:theoremReport(ledger,record).find(x=>x.name===declaration.name)}));for(const x of rows){assert.deepEqual(x.row.by,[declaration.name],'property identity');if(x.row.exhibited&&checker)assert.equal(x.row.holds,true,'property/'+short);}const exhibits=rows.filter(x=>x.row.exhibited);if(checker)assert(exhibits.length>0,'property exhibit/'+short);propertyCoverage.push({name:declaration.name,status:'STATED / sorryAx',coverage:checker?'controlled-check':exhibits.length?'exhibits-only':'gap',exhibits:exhibits.map(x=>x.record.id),notes:checker?'Finite consequent checks only.':exhibits.length?'Action exhibit; full quantified statement not checked.':'No executable nonvacuous exhibit yet; statement remains admitted.'});propertyExecuted++;}
