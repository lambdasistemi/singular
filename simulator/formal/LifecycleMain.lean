...
The wire vectors are pinned twice on purpose: here, and vendored offchain with
their provenance. A row that exports `ok` alone leaves the vendored copy with
nothing to be compared against, so the drift check between the two passes by
having no shared field rather than by agreeing. Every value the vendored module
pins therefore appears here. -/
def wireRow (id : String) (ok : Bool) (fields : List (String × Json)) : Json :=
  Json.mkObj ([ ("id", toJson id), ("ok", toJson ok) ] ++ fields)

def wireRows : List Json :=
  [ wireRow "WD01-fixture-datum-roundtrip"
      (decodeNamingDatum (encodeNamingDatum aliceFixture) == some aliceFixture)
      [ ("expectedBytes", toJson (serialiseNamingDatum aliceFixture))
      , ("fixture", toJson aliceFixture)
      , ("shape", toJson (namingDatumShape (encodeNamingDatum aliceFixture))) ]
  , wireRow "WD02-two-destinations-refused"
      (decodeNamingDatum twoDestinationDatum == none)
      [ ("encoded", wireDataJson twoDestinationDatum)
      , ("malformedBytes", toJson (serialiseWireData twoDestinationDatum)) ] ]
