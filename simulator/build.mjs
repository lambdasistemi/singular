...
import {bound} from './identity.mjs';
const root=new URL('./',import.meta.url),read=p=>readFileSync(new URL(p,root),'utf8');
const identity=JSON.parse(read('identity.json'));const hash=x=>createHash('sha256').update(x).digest('hex');
// The bound set is derived from the shipped mirror, so a file added there and
// left out of identity.json fails here instead of going unhashed.
const expectedKeys=bound().sort().join(',');
if(Object.keys(identity.files).sort().join(',')!==expectedKeys)
  throw Error(`identity binds ${Object.keys(identity.files).length} files, the mirror holds ${bound().length}: run node simulator/identity.mjs`);
...
const js=[module('core.mjs'),module('actions.mjs'),module('properties.mjs'),module('naming.mjs'),module('lifecycle.mjs'),`const STORIES=${read('stories.json')};const CORPUS=${read('corpus.json')};const THEOREMS=${read('formal/theorem-debt.json')};const IDENTITY=${read('identity.json')};const NAMINGCORPUS=${read('../lean/naming-corpus.json')};const NAMINGTHEOREMS=${read('../lean/naming-theorem-debt.json')};const LIFECYCLECORPUS=${read('../lean/lifecycle-corpus.json')};`,read('template-generic.js'),read('page.mjs')].join('\n');
